...
if (_healer call opt_addons_fnc_isMedic) then {
	...
	[tcb_ais_medequip_array, [[1,_defi]]] call opt_addons_fnc_arrayPush;
	...
	if (damage _injured >= 0.5 && {(random 2) >= 1}) then {
		...
		[tcb_ais_medequip_array, [[0,_bb]]] call opt_addons_fnc_arrayPush;
	};
};
...
for "_i" from 1 to (1 + (round random 3)) do {
	...
	if (_i > 1) then {
		[tcb_ais_medequip_array, [[0,_band]]] call opt_addons_fnc_arrayPush;
	} else {
		[tcb_ais_medequip_array, [[1,_band]]] call opt_addons_fnc_arrayPush;
	};
};
if (random 2 >= 1) then {
	...
	[tcb_ais_medequip_array, [[0,_ab]]] call opt_addons_fnc_arrayPush;
};
