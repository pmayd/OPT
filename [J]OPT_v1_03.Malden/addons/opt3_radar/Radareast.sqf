...
private _container = RADARContainerEAST;
private _size = 3500;
private _markerwest = [];
private _markerost = [];
...
for "_i" from 1 to 10 do {
	private _marker2 = createMarkerLocal [format["E%1AIR%2", _i, east],[0,0]];
...
private _Radarring = createMarkerLocal [format["Radarring%1", east],[0,0]];
...
["OPT_RADAR_EAST_ACTIVE", "onEachFrame"] call BIS_fnc_removeStackedEventHandler;
...
if (!isNil "Radarturmeast") then {
    deleteVehicle Radarturmeast;
};
if (!isNil "Radarnetzeast") then {
