...
if (_injured getVariable ["ais_unconscious", false]) then {
	...
	if (isPlayer _healer) then {
		_txt = if (_injured getVariable ["ais_stabilized", false]) then {
			"Der Verwundete muss zuerst wiederbelebt werden."
		} else {
			"Der Verwundete muss zuerst stabilisiert."
		};
		[_txt, "yellow"] remoteExecCall ["AIS_Core_fnc_dynamicText", _healer, false];
	} else {
