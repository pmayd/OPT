...
if (_side == civilian) then 
    ...
    if (_side == west) then 
        ...
        else    
        {
            ...
            _index = lbAdd [10015, "Leer"];    
            ctrlSetText [10042, "Tranung"]            ctrlSetText [10042, "Tranung"];
...
if (OPTWWbuygo == 1) then 
    ...
    if (_side == civilian) then 
        ...
        if ((_bewaffnungpreis -_wert) > 0) then 
            ...
            ["Ausrüsten", format["%1: %2", name player, _txt], "blue"] remoteExecCall [QEFUNC(gui,message), GVAR(textslotwest), false];
